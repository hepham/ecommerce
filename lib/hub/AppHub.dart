...
class AppHub {
  ...
  static HubConnection hubConnection =
      HubConnectionBuilder().withUrl(ApiUrlChatRealTime).build();
  ...
  static OnCreate() async {
    ...
    hubConnection.on("ReceiveMessage", (args) {
      ...
      NewMessageResponse newmessage =
          NewMessageResponse.fromJson(jsonDecode(res));
      Message mess;
      if (newmessage.userId != user.id) {
        mess = new Message(
            receive: user,
            send: user1,
            time: newmessage.chat.createDate.toString(),
            text: newmessage.chat.content);
